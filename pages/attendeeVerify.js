...
class AttendeeVerify extends Component {
  ...
  handleChange(e) {
    if (
      !(
        ...
        this.initialText.toLowerCase().substr(0, e.target.value.length)
      )
    ) {
      this.setState({
        ...this.state,
  ...
  successNotify() {
    ...
    setTimeout(() => {
      window.location =
        window.location.protocol + '//' + window.location.host + '/choose'
    }, 4000)
  }
  ...
  async submitHandler() {
    const eventData = {
      ...
      currentTypingPattern: this.tdna.getTypingPattern({
        type: 0,
        length: 150,
      }),
    ...
    this.FBRef.where('user', '==', this.state.user.email)
      ...
      .then(function(querySnapshot) {
        querySnapshot.forEach(async function(doc) {
          ...
          that.postRequestAndMatchTypingPatterns(
            dbTypingPattern,
            eventData.currentTypingPattern
          )
        })
      })
      .catch(function(error) {
  ...
  render() {
    ...
    const tester =
      this.initialText.substr(0, lenTyped).toLowerCase() ===
      this.state.ksdTest.toLowerCase()
    return (
      <React.Fragment>
                ...
                <Paper className={classes.root} elevation={2}>
                  ...
                  <Typography variant="h6" className={classes.head}>
                    <span
                      style={
                        tester
                          ? { color: 'white', backgroundColor: 'midnightblue' }
                          : { color: 'midnightblue' }
                      }
                    >
